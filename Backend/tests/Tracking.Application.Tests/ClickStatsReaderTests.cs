...
public class ClickStatsReaderTests
{
    ...
    public async Task GetAsync_WithConversions_ReturnsCorrectConvertedCount()
    {
        ...
        db.ClickEvents.AddRange(
            new ClickEvent { AffiliateId = 1, SessionId = "S1", IsUnique = true, ClickedAt = now.AddDays(-1), CreatedAt = now, UpdatedAt = now },
            new ClickEvent { AffiliateId = 1, SessionId = "S2", IsUnique = true, ClickedAt = now.AddDays(-2), CreatedAt = now, UpdatedAt = now },
            new ClickEvent { AffiliateId = 1, SessionId = "S3", IsUnique = true, ClickedAt = now.AddDays(-3), CreatedAt = now, UpdatedAt = now }
        );
